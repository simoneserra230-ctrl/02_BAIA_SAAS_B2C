...
# ── Provider attivo (groq | anthropic) — sovrascrivibile a runtime via API ──
_active_provider: str = os.environ.get("AI_PROVIDER", "groq").lower()
MODEL                 = GROQ_MODEL  # alias retrocompatibile (aggiornato dinamicamente)
CHUNK_SIZE            = 60_000
...
if not GROQ_API_KEY and not ANTHROPIC_API_KEY:
    print("[WARNING] Nessuna API key configurata (GROQ_API_KEY / ANTHROPIC_API_KEY) — le analisi AI falliranno!")
else:
    if GROQ_API_KEY:
        print(f"[STARTUP] Groq configurato ✓  modello={GROQ_MODEL}")
    if ANTHROPIC_API_KEY:
        print(f"[STARTUP] Anthropic configurato ✓  modello={ANTHROPIC_MODEL}")
    print(f"[STARTUP] Provider attivo: {_active_provider}")
...
# ══════════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════════
# AI PROVIDERS  —  Groq + Anthropic + dispatcher unificato
# ══════════════════════════════════════════════════════════════════
...
async def anthropic_call(
    ...
    """
    Chiama Anthropic Claude con retry automatico su rate-limit 429.
    Ritorna (testo_risposta, secondi_attesi_per_rate_limit).
    """
    headers = {
    ...
    async with httpx.AsyncClient(timeout=timeout) as client:
        ...
        if r.status_code == 429 and _retry < 3:
            wait = float(r.headers.get("retry-after", 20)) + 1.0
            print(f"rate limit → attendo {wait:.1f}s (tentativo {_retry+1}/3)")
            await asyncio.sleep(wait)
            result, prev_wait = await anthropic_call(prompt, json_mode, timeout, _retry + 1)
            return result, wait + prev_wait
        ...
        if r.status_code != 200:
            try:
                ...
                detail = r.text
            raise RuntimeError(f"Anthropic error {r.status_code}: {detail}")
...
async def ai_call(
    ...
    """
    Instrada la chiamata al provider attivo (groq | anthropic).
    Usa _active_provider come sorgente di verità a runtime.
    """
    global _active_provider
    if _active_provider == "anthropic":
        if not ANTHROPIC_API_KEY:
            raise RuntimeError("ANTHROPIC_API_KEY non configurata. Inseriscila nelle Impostazioni.")
        return await anthropic_call(prompt, json_mode, timeout)
    else:
        if not GROQ_API_KEY:
            raise RuntimeError("GROQ_API_KEY non configurata. Inseriscila nelle Impostazioni.")
        return await groq_call(prompt, json_mode, timeout)
...
async def prompt_endpoint(
    ...
    try:
        ...
        result, wait_secs = await ai_call(req.prompt, json_mode=json_mode)
...
async def enrich_fields_endpoint(
    ...
    try:
        result, _ = await ai_call(prompt, json_mode=True, timeout=150)
...
@app.get("/model")
def get_model():
    active_model = ANTHROPIC_MODEL if _active_provider == "anthropic" else GROQ_MODEL
    return {
...
async def set_provider(payload: ProviderSettings):
    """
    Cambia il provider AI attivo a runtime (senza riavvio).
    provider: "groq" | "anthropic"
    """
    global _active_provider
    if payload.provider not in ("groq", "anthropic"):
        raise HTTPException(status_code=400, detail="provider deve essere 'groq' o 'anthropic'")
    if payload.provider == "groq" and not GROQ_API_KEY:
        raise HTTPException(status_code=400, detail="GROQ_API_KEY non configurata sul server")
    if payload.provider == "anthropic" and not ANTHROPIC_API_KEY:
        raise HTTPException(status_code=400, detail="ANTHROPIC_API_KEY non configurata sul server")
    _active_provider = payload.provider
    active_model = ANTHROPIC_MODEL if _active_provider == "anthropic" else GROQ_MODEL
    print(f"[CONFIG] Provider cambiato → {_active_provider} ({active_model})")
    return {"ok": True, "provider": _active_provider, "model": active_model}
